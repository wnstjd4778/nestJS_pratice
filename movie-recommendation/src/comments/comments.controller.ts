...
export class CommentsController {
  ...
  findAllBySurveyFormId(@Paging() paging: IPaging, @Param('surveyFormId') surveyFormId: string) {
    return this.commentsService.findAllBySurveyFormId(paging, surveyFormId);
  }
  @Post('/survey-forms/:surveyFormId')
  createComment(@User() user: IAccessTokenPayload, @Param("surveyFormId") surveyFormId: string, @Body() dto: CreateCommentDto) {
    return this.commentsService.createComment(user._id, surveyFormId, dto);
  }

  @Delete('/:commentId')
  deleteComment(@User() user: IAccessTokenPayload, @Param(':commentId') commentId: string) {
    return this.commentsService.deleteComment(user._id, commentId);
  }

  @Put('/:commentId')
  updateComment(@User() user: IAccessTokenPayload, @Param(":commentId") commentId: string, @Body() dto: UpdateCommentDto) {
